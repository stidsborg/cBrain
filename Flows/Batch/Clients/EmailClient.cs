...
{
    Task SendOrderConfirmation(Guid customerId, string trackAndTrace, IEnumerable<Guid> productIds);
}

public class EmailClientStub(ILogger<EmailClientStub> logger) : IEmailClient
{
    public Task SendOrderConfirmation(Guid customerId, string trackAndTrace, IEnumerable<Guid> productIds)
